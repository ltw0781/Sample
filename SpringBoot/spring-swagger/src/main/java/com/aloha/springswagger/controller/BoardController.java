...
public class BoardController {
     ...
     */
    @GetMapping("/{no}")
    public ResponseEntity<?> getOneBoard(@Parameter(name = "no", description = "글번호를 입력하세요.", required = true) @PathVariable("no") Integer no) {
        try {
    ...
    public ResponseEntity<?> createBoard(@Parameter(description = "제목, 작성자, 내용을 입력하세요.") @RequestBody Board board) {
        try {
     ...
     * 요청 실패 : 400 Bad Request OR 500 Internal Service Error "FAIL"
     * 
     * @param board
     * @return
     */
