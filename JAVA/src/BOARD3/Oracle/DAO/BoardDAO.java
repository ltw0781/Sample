...
public class BoardDAO extends JDBConnection{
	...
	public List<Board> list(){
		...
		String sql = " SELECT *"
				   + " FROM board";
		try {
			// 쿼리(SQL) 실행객체 생성		- Statement (stmt)
			stmt = con.createStatement();
			
			// 쿼리(SQL) 실행 --> 결과		- ResultSet (rs)
			rs = stmt.executeQuery(sql);
			
			// 조회결과를 리스트(boardList)에 추가
	...
	public Board select(int no){
		...
		String sql = " SELECT * "
				   ...
				   + " WHERE no = ? ";
		try {
			// 쿼리(SQL) 실행객체 생성		- PreparedStatement (psmt)
			psmt = con.prepareStatement(sql);
			
			// psmt.setXXX(순서번호, 매필할 값);
			psmt.setInt(1, no); 		// ?(1) <-- no (글 번호)
