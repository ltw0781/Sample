...
public class CalendarEx2 {
	...
	public static void main(String[] args) {
		...
		// sdf.format(Date 객체)
		// * 지정한 날짜 형식에 맞게 Date 객체의 날짜/시간정보를 문자열로 변환하는 메소드
		...
		// 10분 후
		// cal.add( 시간단위 상수, 시간 값)
		// : 저정한 시간 단위만큼 이후의 날짜/시간으로 변경하는 메소드
		...
		// 
		// 조건
		// 일요일 6시 ~ 토요일 8시 전까지			: 이번주 토요일 오후 9시
		...
		// 시간은 고정된 값 : 9시, 0분, 0초
		lottoCal.set(Calendar.AM_PM, 1); // 오전_오후의 값은 --> 오전 : 0, 오후 : 1
		lottoCal.set(Calendar.HOUR, 9); // 시간
		lottoCal.set(Calendar.MINUTE, 0); // 분
		lottoCal.set(Calendar.SECOND, 0); // 초
		
		// 1. 구입 가능 - 추첨일 출력 하는 방법
		
		// 2. 구입 불가 - 제외 하는 방법 제외를 하고 나머지 출력
		...
		if(ampm == 0 && 시간 >= 0 && 시간 <= 6) {
			...
			System.err.println("구입가능한 시간이 아닙니다.");
		}
		// (구입불가) 2. 토요일이면서, 오후 8시 이후
